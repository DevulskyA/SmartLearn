...
export async function extractSource(db, userId, sourceId, { sourcesDir, deadlineMs = 30_000, memoryLimitMb = 256 }, now = () => new Date()) {
  ...
  const applied = db.transaction(() => {
    ...
    if (result.status === 'EXTRACTED') {
      ...
      const insertOutline = db.prepare('INSERT INTO source_outline (user_id, source_id, ordinal, level, title, page_index, detected) VALUES (?, ?, ?, ?, ?, ?, ?)');
      (result.outline ?? []).forEach((entry, ordinal) => insertOutline.run(userId, sourceId, ordinal, entry.level, entry.title, entry.pageIndex, entry.detected ? 1 : 0));
      db.prepare(`
