...
// ---------------------------------------------------------------------------
// BrowserStore → Broker migration helpers
// ---------------------------------------------------------------------------

const BROWSER_STORE_KEY = "smartlearn:browser-db";

function hasBrowserStoreData() {
  try {
...
function buildMigrationStatements(state) {
  const stmts = [];
  ...
  for (const t of state.reviewTasks ?? []) {
    stmts.push({
      ...
      params: [t.id, t.studyRecordId, t.reviewNumber, t.dueDate, t.completedAt ?? null, t.reviewDone ? 1 : 0, t.questionsDone ? 1 : 0, t.questionsCount ?? null, t.correctCount ?? null, t.scorePercent ?? null, t.comment ?? null, t.createdAt, t.updatedAt],
    });
  }
  return stmts;
}
