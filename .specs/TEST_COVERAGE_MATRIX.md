...
|---|---|---|---|
| app.js | 5105 | No dedicated unit test (expected — DOM orchestrator). Exercised across ~20 e2e specs (practice, exercise-attempt-review, stats-*, atomic-save, offline*, migration, local-authority, feature-parity, smartlearn-plan-flow, select-ui, context-switcher-keyboard, source-proposals, draft-acceptance, study-now-flow) | PARTIAL — huge surface, e2e-only, no isolated logic extraction; a change deep in one screen's handler has no fast focused gate, only the full e2e suite |
