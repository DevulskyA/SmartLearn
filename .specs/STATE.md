...
T51 PARTIAL (AGENT_CLI, 268d9c6): code+tests done (server/src/production-config.js fail-closed startup, server/scripts/check-production-config.mjs, scripts/release-manifest.mjs, static secret + Tauri capability/CSP review tests; server 433/433, root 378/378). PENDING before T51 can be DONE: deployment/security runbook (one-origin HTTPS, proxy trust, env table, secrets injection, budgets, ownership, missing TLS/domain/signing/provider list), main-protection/check policy proposal (NOT applied; needs admin approval), Windows/Android package builds (NOT_PROVEN, never run here), tasks.md T51 boxes + validation.md row. Known CSP relaxations pinned by test: style-src 'unsafe-inline', connect-src https: http://localhost:*.
