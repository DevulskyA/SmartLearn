...
NEXT_STEP: T22 (complete screen and settings parity without losing old data) — finish Hoje/Plano/Disciplinas/Estatisticas/Acompanhamento/Configuracoes against RemoteStore, resolve the obsolete-Cadastro-route question, replace dangerous local reset affordances in server mode with an explicit supported operation, offer legacy export until migration completes. No blockers. Independent Fresh Verifier for the whole Phase 03 server-authoritative-Web slice is explicitly assigned to T24, not before.
